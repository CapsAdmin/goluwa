...
do
	...
	event.Timer(
		...
		function()
			...
			if
				WINDOW and
				battery_limit:Get() and
				system.IsUsingBattery() and
				system.GetBatteryLevel() < 0.95
			then
				--  render.GetWindow():SwapInterval(true)
				if system.GetBatteryLevel() < 0.20 then rate = 10 end

				if not window.IsFocused() then rate = 5 end
			end

			if SERVER then rate = 66 end

			if suppress_limit > system.GetElapsedTime() then rate = 45 end
		end
	)

	event.AddListener("FrameEnd", "fps_limit", function()
		if rate > 0 then system.Sleep(1 / rate) end
	end)
end