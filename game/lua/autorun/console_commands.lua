commands.Add("bootime", function()
	...
	logn(
		"spent ",
		os.clock() - e.BOOTIME,
		" seconds from core/lua/init.lua to this command"
	)
end)
...
commands.Add("debug=string", function(lib)
	...
	if type(tbl) == "table" then
		...
		if tbl.EnableDebug then tbl.EnableDebug(tbl.debug) end
...
commands.Add("lfind=string", function(what)
	for path, lines in pairs(utility.FindInLoadedLuaFiles(what)) do
		...
		for _, info in ipairs(lines) do
			...
			logf("\t%d: %s\n", info.line, str)
			logn(
				(" "):rep(#tostring(info.line) + 5 + info.start),
				("^"):rep(info.stop - info.start + 1)
			)
		end
	end
end)
