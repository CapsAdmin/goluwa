...
do -- PUT ME IN TRANSFORM
	...
	local function get_axes(ang)
		return ang:GetForward(), -ang:GetRight(), ang:GetUp()
	end
	...
	function mctrl.Move(axis, mouse_pos)
		...
		if target:IsValid() then
			...
			elseif axis == "z" then
				...
				local localpos = math3d.LinePlaneIntersection(pos, forward, screen_pos) or
					math3d.LinePlaneIntersection(pos, right, screen_pos)

				if localpos then
					final = get_target_position(pos + localpos:GetDot(up) * up - up * mctrl.size, ang)
				end
			elseif axis == "view" then
				local localpos = math3d.LinePlaneIntersection(pos, render3d.camera:GetAngles():GetForward(), mouse_pos)

				if localpos then final = get_target_position(pos + localpos, ang) end
			end
		...
		if target:IsValid() then
			...
			if axis == "x" then
				...
				if localpos then
					...
					if diffang.y < 0 then diffang.x = -diffang.x + math.pi end
			...
			if final and final:IsValid() then target:SetRotation(final * rot) end
		--[[
...
commands.Add("close_editor=nil", editor.Close)
commands.Add("toggle_editor=nil", editor.Toggle)
