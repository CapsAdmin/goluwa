...
local function calc_job(id, job, play_next_now)
	if job.sound_queue and #job.sound_queue > 0 then
		...
		if play_next_now or job.next_sound < system.GetElapsedTime() then
			...
			job.next_sound = system.GetElapsedTime() + data.duration
		end
	else
		job.sound_queue = nil
		job.next_sound = nil
	...
	for chunk in (" " .. text .. " ."):gsub("%p", " %1 "):gmatch("(.-)[%s]") do
		if chunk:trim() ~= "" then
			if pause_symbols[chunk] then
				...
				if not cutoff then cutoff = (-0.25 * (1 + (#chunk / 10))) / 1.25 end
...
local function stop_speaking(job)
	if ent.current_sound then ent.current_sound:Stop() end
...
local function add_voices(path, name)
	...
	for k, v in pairs(vfs.Find("sound/" .. path .. ".+%.wav")) do
		if
			...
			not v:lower():find("hurt")
			and
			not v:lower():find("die")
			and
...
for i = 1, 7 do
	voices.bird[i] = "ambient/levels/coast/coastbird" .. i .. ".wav"
end

voices.robot = {}

for i = 1, 6 do
	voices.robot[i] = "ambient/levels/canals/headcrab_canister_ambient" .. i .. ".wav"
end

voices.terminal = {}

for i = 1, 4 do
	voices.terminal[i] = "ambient/machines/combine_terminal_idle" .. i .. ".wav"
end

voices.bird_swamp = {}

for i = 1, 6 do
	voices.bird_swamp[i] = "ambient/levels/canals/swamp_bird" .. i .. ".wav"
end

voices.bird_coast = {}

for i = 1, 7 do
	voices.bird_coast[i] = "ambient/levels/coast/coastbird" .. i .. ".wav"
end
...
for i = 1, 14 do
	voices.zombie[i] = "npc/zombie/zombie_voice_idle" .. i .. ".wav"
end

voices.cat = {}

for i = 1, 3 do
	voices.cat[i] = "items/halloween/cat0" .. i .. ".wav"
end

_G.speaker = {
	PlayPhrase = function(id, text, list, ...)
		if type(list) == "string" then list = voices[list] end
