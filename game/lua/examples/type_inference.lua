...
local function is_type(val, typ)
	...
	for _, str in ipairs(val.value_types) do
		if str == typ then return true end
	end
...
local function push_scope()
	...
	if parent then table.insert(parent.children, scope) end
end
...
local function walk(node)
	if node.type == "assignment" then
		...
		if node.sub_type == "function" then
			if node.value.index_expression.type == "value" then
				set_variable(
					node.value.index_expression.value,
					{
						type = "constant",
						value = node.value,
						value_types = {node.value.type},
	...
	elseif node.type == "for" then
		...
		if node.val then walk(node.val) end
	...
	elseif node.type == "if" then
		for _, clause in ipairs(node.clauses) do
			...
			if clause.expr then branch_evaluation = nil end
		end
	elseif node.type == "value" then
		...
		if
			node.value.type == "letter" and
			node.value.value ~= "true" and
			node.value.value ~= "false" and
...
local function expand_operator(val)
	if is_type(val, "operator") then
		...
		if val.left then str = str .. value2string(val.left) end

		str = str .. " " .. val.value.operator .. " "

		if val.right then str = str .. value2string(val.right) end
