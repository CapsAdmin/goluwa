    ...
    if path:endswith(".lua") then
        ...
        if f then
            ...
            if content and loadstring(content) then
                vfs.Write("os:" .. e.ROOT_FOLDER .. "harvest_lua/flat/" .. crypto.CRC32(content) .. ".lua", content)
            end
        end
    end
end
...
for _, path in ipairs(fs.get_files_recursive("harvest_lua")) do
    if path:endswith(".lua") then
        local f = assert(io.open(path)) local content = f:read("*all") f:close()
        if content then
            vfs.CopyFile("os:" .. e.ROOT_FOLDER .. path, "os:" .. e.ROOT_FOLDER .. "harvest_lua/flat/" .. crypto.CRC32(content) .. ".lua")
        end
    end
end