...
function sqlite_db:close()
	...
	if r == sqlite3.SQLITE_OK then self.db = nil else self:check(r) end
end
...
function sqlite_db:isopen()
	return self.db and true or false
end
...
function sqlite_db:dump_unfinalized_statements()
	for _, stmt in pairs(self.stmts) do
		...
		if stmt.trace then print("defined at: " .. stmt.trace) end
	end
end
...
function sqlite_db:dump_unclosed_blobs()
	for _, blob in pairs(self.blobs) do
		...
		if blob.trace then print("defined at: " .. blob.trace) end
	end
end
...
function sqlite_stmt:isopen()
	return self.stmt and true or false
end
...
function sqlite_stmt:rows()
	return function()
		if self:step() then return self:get_values() else return nil end
	end
end
...
function sqlite_blob:write(offset, data, datalen)
	if type(data) == "string" then
		...
		assert(datalen and type(datalen) == "number")
	end
