...
local function field(obj, name, line)
	return build(
		"MemberExpression",
		{object = obj, property = ident(name), computed = false, line = line}
	)
end
...
function AST.func_parameters_decl(ast, args, vararg)
	...
	if vararg then params[#params + 1] = ast:expr_vararg() end
...
function AST.expr_index(ast, v, index, line)
	return build(
		"MemberExpression",
		{object = v, property = index, computed = true, line = line}
	)
end
...
function AST.expr_property(ast, v, prop, line)
	...
	return build(
		"MemberExpression",
		{object = v, property = index, computed = false, line = line}
	)
end
...
function AST.if_stmt(ast, tests, cons, else_branch, line)
	return build(
		"IfStatement",
		{tests = tests, cons = cons, alternate = else_branch, line = line}
	)
end
...
local function new_ast()
	local match_id_name = function(id, name)
		return id.name == name
	end
