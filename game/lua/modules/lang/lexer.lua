...
local function error_lex(chunkname, tok, line, em, ...)
	...
	if tok then msg = string.format("%s near '%s'", msg, tok) end
...
local function fillbuf(ls)
	...
	if not data then return END_OF_STREAM end
...
local function read_long_string(ls, sep, ret_value)
	...
	if ret_value then return get_string(ls, 2 + sep, 2 + sep) end
end
...
local function lex_setup(read_func, chunkname)
	...
	if
		...
		byte(ls, 0) == "\xbb" and
		byte(ls, 1) == "\xbf"
	then -- Skip UTF-8 BOM (if buffered).
		ls.n = ls.n - 2
		ls.p = ls.p + 2
		nextchar(ls)
		header = true
