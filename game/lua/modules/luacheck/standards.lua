...
local function validate_fields(fields, is_root, index)
	...
	if type(fields) ~= "table" then
		return false,
		("%ss table expected, got %s"):format(field_type, type(fields)),
		index
	end
	...
	for key, value in pairs(fields) do
		if type(key) == "string" then
			...
			if type(value) ~= "table" then
				return false,
				("%s description table expected, got %s"):format(field_type, type(value)),
				new_index
			end
			...
			if not ok then return false, err, err_index end
		elseif type(value) ~= "string" then
			...
			return false,
			("string expected as %s name, got %s"):format(field_type, type(value)),
			new_index
		end
	end
...
function standards.overwrite_field(final_std, field_names, read_only)
	...
	for _, field_name in ipairs(field_names) do
		if not field_def.fields then field_def.fields = {} end
...
local function infer_deep_read_only_statuses(def, read_only)
	...
	if deep_read_only then def.deep_read_only = true end
end
