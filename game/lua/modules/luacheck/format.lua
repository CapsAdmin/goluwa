...
local color_codes = {reset = 0, bright = 1, red = 31, green = 32}
...
local function format_file_report_header(report, file_name, opts)
	...
	elseif #report == 0 then
		...
		if errors > 0 then
			...
			status = status .. (
					format_color(tostring(errors) .. " error" .. plural(errors), opts.color, "bright")
				)
		end
	end
...
local function format_location(file, location, opts)
	...
	if opts.ranges then res = ("%s-%d"):format(res, location.end_column) end
...
function format.builtin_formatters.default(report, file_names, opts)
	...
	local total = (
		"Total: %s warning%s / %s error%s in %d file%s"
	):format(
		format_number(report.warnings, opts.color),
		plural(report.warnings),
		format_number(report.errors, opts.color),
	...
	if report.fatals > 0 then
		total = total .. (
				", couldn't check %s file%s"
			):format(report.fatals, plural(report.fatals))
	end
...
function format.builtin_formatters.TAP(report, file_names, opts)
	...
	for i, file_report in ipairs(report) do
		if file_report.fatal then
			table.insert(
				buf,
				("not ok %d %s: %s"):format(#buf + 1, file_names[i], fatal_type(file_report))
			)
		elseif #file_report == 0 then
			...
			for _, warning in ipairs(file_report) do
				table.insert(
					buf,
					("not ok %d %s"):format(#buf + 1, format_event(file_names[i], warning, opts))
				)
			end
		end
	end
	...
	for file_i, file_report in ipairs(report) do
		if file_report.fatal then
			table.insert(
				...
				):format(escape_xml(file_names[file_i]), escape_xml(file_names[file_i]))
			)
		...
		elseif #file_report == 0 then
			...
			for event_i, event in ipairs(file_report) do
				table.insert(
					buf,
					(
						[[    <testcase name="%s:%d" classname="%s">]]
					):format(escape_xml(file_names[file_i]), event_i, escape_xml(file_names[file_i]))
				)
				table.insert(
					buf,
					(
						...
						escape_xml(event_code(event)),
						escape_xml(format_event(file_names[file_i], event, opts))
					)
				)
				table.insert(buf, [[    </testcase>]])
			end
...
local fatal_error_codes = {["I/O"] = "F1", ["syntax"] = "F2", ["runtime"] = "F3"}
	...
	for i, file_report in ipairs(report) do
		if file_report.fatal then
			...
			table.insert(
				buf,
				(
					"luacheck : fatal error %s: couldn't check %s: %s"
				):format(fatal_error_codes[file_report.fatal], file_names[i], file_report.msg)
			)
		else
			for _, event in ipairs(file_report) do
				...
				table.insert(
					...
					(
						"%s(%d,%d) : %s %s: %s"
					):format(file_names[i], event.line, event.column, event_type, event_code(event), message)
				)
			end
		end
	end
...
function format.builtin_formatters.plain(report, file_names, opts)
	...
	for i, file_report in ipairs(report) do
		if file_report.fatal then
			table.insert(
				buf,
				("%s: %s (%s)"):format(file_names[i], fatal_type(file_report), file_report.msg)
			)
		else
			for _, event in ipairs(file_report) do
...
function format.format(report, file_names, options)
	return format.builtin_formatters[options.formatter or
	"default"](
		report,
		file_names,
		{
