...
stage.warnings = {
	["211"] = {
		message_format = unused_local_message_format,
	...
	["331"] = {
		message_format = "value assigned to variable {name!} is mutated but never accessed",
		fields = {"name", "secondary"},
	},
}
...
local type_codes = {var = "1", func = "1", arg = "2", loop = "3", loopi = "3"}

local function warn_unused_var(chstate, value, is_useless)
	chstate:warn_value(
		"21" .. type_codes[value.var.type],
		value,
		{
...
local function warn_unaccessed_var(chstate, var, is_mutated)
	...
	chstate:warn_var(
		"2" .. (is_mutated and "4" or "3") .. type_codes[var.type],
		var,
		{secondary = secondary}
	)
end
...
local function warn_unused_value(chstate, value, overwriting_node)
	local warning = chstate:warn_value(
		"3" .. (value.mutated and "3" or "1") .. type_codes[value.type],
		value,
		{secondary = is_secondary(value) or nil}
	)
...
local function detect_unused_local(chstate, var)
	if is_function_var(var) then
		...
		if not value.used then warn_unused_var(chstate, value) end
