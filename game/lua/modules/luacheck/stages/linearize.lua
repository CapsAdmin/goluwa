...
local function warn_redefined(chstate, var, prev_var, is_same_scope)
	local code = "4" .. (
			is_same_scope and
			"1" or
			var.line == prev_var.line and
			"2" or
			"3"
		) .. type_codes[prev_var.type]
	chstate:warn_var(
		code,
		var,
		{
...
local function warn_unused_label(chstate, label)
	chstate:warn_range("521", label.range, {label = label.name})
end
...
function Line:walk(visited, index, callback, ...)
	...
	if callback(self, index, item, ...) then return end
...
local function new_label(line, name, range)
	return {name = name, range = range, index = line.items.size + 1}
end

local function new_goto(name, jump, range)
	return {name = name, jump = jump, range = range}
end
...
local function new_noop_item(node, loop_end)
	return {tag = "Noop", node = node, loop_end = loop_end}
end
...
function LinState:emit_stmt_If(node)
	...
	if #node % 2 == 1 then self:emit_block(node[#node]) end
