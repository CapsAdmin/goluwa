...
stage.warnings = {
	["571"] = {
		message_format = "numeric for loop goes from #(expr) down to {limit} but loop step is not negative",
		fields = {"limit"},
	},
}

local function check_fornum(chstate, node)
	if node[2].tag ~= "Op" or node[2][1] ~= "len" then return end
	...
	if node[5] then step = core_utils.eval_const_node(node[4]) end

	if step and step >= 0 then
		chstate:warn_range("571", node, {limit = limit_repr})
	end
end
