...
local function match(pattern, code, name)
	...
	if code_pattern then matches_code = utils.pmatch(code, code_pattern) end
...
local function passes_rules_filter(rules, code, name)
	...
	for _, rule in ipairs(rules) do
		...
		for _, pattern in ipairs(rule[1]) do
			...
			if enabled_code then matches_code = rule[2] ~= "disable" end

			if enabled_name then matches_code = rule[2] ~= "disable" end
			...
			if rule[2] == "enable" then
				if matches_code then enabled_code = true end

				if matches_name then enabled_name = true end
...
local function passes_filter(normalized_options, warning)
	...
	elseif warning.code:find("^1[14]") then
		...
		if
			not warning.module and
			get_field_status(normalized_options, warning) ~= "undefined"
		then
			return false
		end
	end
	...
	if warning.self and not normalized_options.self then return false end
...
function CachingOptionsNormalizer:normalize_options(stds, option_stack)
	...
	if result_node.result then return result_node.result end
...
local function filter_not_global_related_in_file(check_result, options_normalizer, stds, option_stack)
	...
	for line in ipairs(check_result.line_lengths) do
		next_inline_option_index = update_option_stack_for_new_line(check_result, stds, option_stack, line, next_inline_option_index)
...
local function is_definition(normalized_options, warning)
	return normalized_options.allow_defined or
		(
			normalized_options.allow_defined_top and
			warning.top
		)
end
...
local function apply_implicit_definitions(globally_defined, globally_used, locally_defined, normalized_options, warning)
	if not warning.code:find("^11") then return warning end

	if warning.code == "111" then
		if normalized_options.module then
			if locally_defined[warning.name] then return end

			warning.module = true
		else
			if is_definition(normalized_options, warning) then
				if globally_used[warning.name] then return end

				warning.code = "131"
				warning.top = nil
			else
				if globally_defined[warning.name] then return end
			end
		end
	else
...
local function filter_global_related_in_file(check_result, globally_defined, globally_used, locally_defined)
	for _, warning in ipairs(check_result.warnings) do
		if warning.code:find("^1") then
			local normalized_options = check_result.normalized_options[warning.line]
			warning = apply_implicit_definitions(globally_defined, globally_used, locally_defined, normalized_options, warning)

			if warning then
				if
					warning.code:find("^11[12]") and
					not warning.module and
					get_field_status(normalized_options, warning) == "read_only"
				then
					warning.code = "12" .. warning.code:sub(3, 3)
				elseif
					warning.code:find("^11[23]") and
					get_field_status(normalized_options, warning, 1) ~= "undefined"
				then
					warning.code = "14" .. warning.code:sub(3, 3)
				end

				if
					warning.code:match("11[23]") and
					get_field_status(normalized_options, warning, 1) ~= "undefined"
				then
					warning.code = "14" .. warning.code:sub(3, 3)
				end
