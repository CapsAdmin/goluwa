...
local function profiler_StopTimer(...)
	if steam.debug or _debug then
		profiler.StopTimer(...)
	end
end
...
function steam.LoadMap(path, callback)
	...
	do 
		profiler_StartTimer("reading lumps") -- lumps
		local struct = [[
	...
	do 
		profiler_StartTimer("mounting pak")-- pak
	...
	if false then  
		profiler_StartTimer("reading game lump")
	...
	do
		profiler_StartTimer("reading entities")
	...
	profiler_StartTimer("reading brushes")
	...
	profiler_StartTimer("reading brushsides")
	...
	profiler_StartTimer("reading texinfo")	
	...
	profiler_StartTimer("reading texdata")
	...
	profiler_StartTimer("reading texdatastringtable")
	...
	do 
		profiler_StartTimer("reading displacements")
		local structure = [[
	...
	profiler_StartTimer("reading models")
	...
	do 
		profiler_StartTimer("building mesh")
	...
	profiler_StartTimer("generating normals")
	for i, data in ipairs(bsp_mesh.sub_models) do
		utility.GenerateNormals(data.vertices)	
	end 
	profiler_StopTimer()

	profiler_StartTimer("smoothing displacements")
	for i, data in ipairs(bsp_mesh.sub_models) do
	...
	if CLIENT then
		profiler_StartTimer("render.CreateMesh")
		for i, data in ipairs(bsp_mesh.sub_models) do
	...
	profiler_StartTimer("creating entities")
