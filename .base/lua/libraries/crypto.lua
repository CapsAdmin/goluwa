...
local function crc8Add(crc,c)
-- 'crc' should be initialized to 0x00.
	return CRC8_TAB[bit.band(bit.bxor(crc,c), 0xFF)]
end
-- C/C++ language:
-- 
...
local function crcArcAdd(crc,c)
-- 'crc' should be initialized to 0x0000.
	return CRC_ARC_TAB[bit.band(bit.bxor(crc,c), 0xFF)]^bit.band((bit.rshift(crc,8)),0xFF)
end
-- C/C++ language:
-- 
...
local function crc16Add(crc,c)
-- 'crc' should be initialized to 0x0000.
	return bit.bxor(CRC16_TAB[bit.band(bit.bxor((bit.rshift(crc,8)), c),0xFF)], bit.band((bit.lshift(crc,8)),0xFFFF))
end
-- C/C++ language:
-- 
...
-- result will always be 0xF0B8 (without the complementation).
	return bit.bxor(FCS_16_TAB[bit.band(bit.bxor(fcs,c),0xFF)], bit.band((bit.rshift(fcs,8)),0xFF))
end
...
-- 
local CRC32_TAB = Array{ --[[ CRC polynomial 0xEDB88320 ]]
	-- C/C++ language:
	-- 
...
-- result will always be 0xDEBB20E3 (without the complementation).
	return bit.bxor(CRC32_TAB[bit.band(bit.bxor(crc,c),0xFF)],bit.band((bit.rshift(crc,8)),0xFFFFFF))
end
--
-- C/C++ language:
-- 
...
local function crc_string(func, init, stop)
	init = init or 0
	return function(str)
		local crc = init	
			
		for i = 1, #str do
			crc = func(crc, str:byte(i))
		end
		
		if stop then
			return bit.bxor(crc, stop)
		else
			return crc
		end
	end
...
crypto.CRC8 = crc_string(crc8Add)
crypto.CRCArc = crc_string(crcArcAdd)
crypto.CRCModbus = crc_string(crcArcAdd, 65535)
crypto.CRC16 = crc_string(crc16Add)
crypto.FCS16 = crc_string(fcs16Add, 0xFFFF, 0xFFFF)
crypto.CRC32 = crc_string(crc32Add, 0xFFFFFFFF, 0xFFFFFFFF)
