...
render3d.AddModelDecoder(
	"assimp",
	function(path, full_path, mesh_callback)
		local flags = bit.bor(tonumber(assimp.e.TargetRealtime_Quality), tonumber(assimp.e.ConvertToLeftHanded))

		assimp.ImportFileEx(
			full_path,
			flags,
			function(model_data, i, total_meshes)
				if render3d.debug then
					llog("%s loading %q %s\n", full_path, model_data.name, i .. "/" .. total_meshes)
				end
