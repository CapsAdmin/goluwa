...
for i, v in pairs(known) do
	...
	if not table.hasvalue(skins, v) then table.insert(skins, v) end
end
	...
	function SKIN:Build()
		...
		texture.OnLoad = function()
			...
			for k, v in pairs(gui.panels) do
				if v:HasSkin(skin.Name) then v:SetSkin(skin) end
			end
		end

		-- FIX ME
		if not texture:IsLoading() then texture:OnLoad() end
