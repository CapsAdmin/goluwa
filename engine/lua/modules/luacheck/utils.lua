...
function utils.read_file(file)
	...
	if not res then return nil, "couldn't read: " .. read_err end

	-- Use :len() instead of # operator because in some environments
	-- string library is patched to handle UTF.
	if res:sub(1, bom:len()) == bom then res = res:sub(bom:len() + 1) end
...
function utils.load_config(path, env)
	...
	if not src then return nil, "I/O", read_err end
...
function utils.after(str, pattern)
	...
	if last_matched_index then return str:sub(last_matched_index + 1) end
end
...
function utils.split_lines(str)
	...
	while true do
		...
		if not line_end_pos then break end
...
function utils.array_of(type_)
	return function(x)
		...
		for _, item in ipairs(x) do
			if type(item) ~= type_ then return false end
		end
