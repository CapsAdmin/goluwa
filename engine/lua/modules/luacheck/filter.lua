...
local function filter_implicit_defs(report)
	...
	for i, file_report in ipairs(report) do
		if not file_report.fatal then
			res[i] = filter_implicit_defs_file(
				file_report,
				info.globally_defined,
				info.globally_used,
				info.locally_defined[i]
			)
		else
			res[i] = file_report
		end
...
local function filters(opts, warning)
	...
	if
		warning.code:match("1[14].") and
		warning.indirect and
		get_field_status(opts, warning, warning.previous_indexing_len) == "undefined"
	then
		return true
	end
	...
	if
		...
		get_field_status(opts, warning) ~= "undefined"
	then
		return true
	end
...
local function filter_file_report(report)
	...
	for _, pair in ipairs(report) do
		...
		if
			issue.code:match("11[12]") and
			not issue.module and
			get_field_status(opts, issue) == "read_only"
		then
