...
local luacheck = {_VERSION = "0.20.0"}

local function raw_validate_options(fname, opts)
	assert(
		opts == nil or type(opts) == "table",
		(
			"bad argument #2 to '%s' (table or nil expected, got %s)"
		):format(fname, type(opts))
	)
	...
	if not ok then
		if invalid_field then
			error(
				(
					"bad argument #2 to '%s' (invalid value of option '%s')"
				):format(fname, invalid_field)
			)
		else
...
function luacheck.get_report(src)
	local msg = (
		"bad argument #1 to 'luacheck.get_report' (string expected, got %s)"
	):format(type(src))
...
function luacheck.process_reports(reports, opts)
	local msg = (
		"bad argument #1 to 'luacheck.process_reports' (table expected, got %s)"
	):format(type(reports))
...
function luacheck.check_strings(srcs, opts)
	local msg = (
		"bad argument #1 to 'luacheck.check_strings' (table expected, got %s)"
	):format(type(srcs))
	assert(type(srcs) == "table", msg)

	for _, item in ipairs(srcs) do
		msg = (
			"bad argument #1 to 'luacheck.check_strings' (array of strings or tables expected, got %s)"
		):format(type(item))
...
function luacheck.check_files(files, opts)
	local msg = (
		"bad argument #1 to 'luacheck.check_files' (table expected, got %s)"
	):format(type(files))
	assert(type(files) == "table", msg)

	for _, item in ipairs(files) do
		msg = (
			"bad argument #1 to 'luacheck.check_files' (array of paths or file handles expected, got %s)"
		):format(type(item))
...
function luacheck.get_message(issue)
	local msg = (
		"bad argument #1 to 'luacheck.get_message' (table expected, got %s)"
	):format(type(issue))
	assert(type(issue) == "table", msg)
	return format.get_message(issue)
end

setmetatable(luacheck, {
	__call = function(_, ...)
