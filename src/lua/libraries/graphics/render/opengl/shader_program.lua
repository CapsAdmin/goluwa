...
function META:SetupStorageBlock(name, bind_location)
	local update_info = {last = {}}
	local struct = "struct {\n"
	local total_size = 0
	local info = self:GetProperties().shader_storage_block[name]
	...
	for k,v in pairs(info.variables) do
		if v.type.size then
			...
			if v.type.name == "float" then
				struct = struct .. "\tfloat " ..  v.name .. ";\n"
				update_info[v.name] = function(ssbo, data, var)
					data[v.name] = var
					ssbo:UpdateData(data[v.name], size, offset)
				end
			else
				local length =  v.type.size / ffi.sizeof("float")
				struct = struct .. "\tfloat " ..  v.name .. "[" .. length .. "];\n"
				if length == 16 then
					update_info[v.name] = function(ssbo, data, var)
						data[v.name] = var:GetFloatPointer()
						ssbo:UpdateData(data[v.name], size, offset)
					end
	...
	self.ssbos = self.ssbos or {}
	self.ssbos[name] = {}

	self.ssbos[name].data = ffi.typeof(struct)()
	self.ssbos[name].update_info = update_info
	self.ssbos[name].ssbo = render.CreateShaderStorageBuffer("dynamic_draw", total_size)
	self.ssbos[name].bind_location = bind_location
	self:BindShaderBlock(info.block_index, bind_location)
end

function META:SetStorageBlockVariable(name, key, var)
	if self.ssbos[name].update_info.last[key] ~= var then
		self.ssbos[name].update_info[key](self.ssbos[name].ssbo, self.ssbos[name].data, var)
		self.ssbos[name].update_info.last[key] = var
	end
end

function META:BindStorageBlock(name)
	self.ssbos[name].ssbo:Bind(self.ssbos[name].bind_location)
end
