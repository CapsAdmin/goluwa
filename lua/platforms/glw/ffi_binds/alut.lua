-- https://github.com/malkia/ufo/blob/master/ffi/OpenALUT.lua
...
local library = ffi.load("alut")

local alut = {}

for line in header:gmatch("(.-)\n") do
	local func_name = line:match(" (alut%u.-) %(")
	if func_name then
		alut[func_name:sub(5)] = function(...) 
			return library[func_name](...)
		end
	end
end

function alut.GetLastError()	
	return ffi.string(alut.GetErrorString(al.GetError()))
end
