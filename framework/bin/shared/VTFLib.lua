...
function library.LoadImage(data, path_hint)
	...
	if library.CreateImage(vtf_image) == 0 then
		return false, "failed to create image: " .. get_error()
	end

	if library.BindImage(vtf_image[0]) == 0 then
		return false, "failed to bind image: " .. get_error()
	end

	-- dummy material
	local vtf_material = ffi.new("unsigned int[1]")

	if library.CreateMaterial(vtf_material) == 0 then
		return false, "failed to create material: " .. get_error()
	end

	if library.BindMaterial(vtf_material[0]) == 0 then
		return false, "failed to bind material: " .. get_error()
	end
	...
	do
		...
		if
			library.ImageConvert(internal_buffer, buffer, width, height, internal_format, conversion_format) == 0
		then
			cleanup(vtf_material, vtf_image)
			return false,
			"conversion from " .. tostring(internal_format) .. " to " .. tostring(conversion_format) .. " failed: " .. get_error()
		end
	end
