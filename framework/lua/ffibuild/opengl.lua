...
local manual_enum_group_fixup = {texture = {
...
local pseduo_objects = {
	...
	Texture = {
		...
		functions = {
			...
			TexBufferRange = {static_arguments = {"GL_TEXTURE_BUFFER"}},
			TexBufferRangeEXT = {static_arguments = {"GL_TEXTURE_BUFFER"}},
			TexBuffer = {static_arguments = {"GL_TEXTURE_BUFFER"}},
		},
	},
}
local enums = {}
...
for value, enum in xml:gmatch("<enum value=\"(.-)\" name=\"(.-)\"") do
	if #value < 18 then enums[enum] = tonumber(value) end
end
	...
	for param in str:gmatch("<param.-</param>") do
		...
		if not group then
			for k, v in pairs(manual_enum_group_fixup) do
				if func_name:lower():find(k, nil, true) then group = v[name] end
			end
		end
		...
		if type == "GLenum" then type = "GL_LUA_ENUMS" end
...
for name, str in xml:gmatch("<require comment=\"(.-) object functions\">(.-)</require>") do
	if not name:find("\n") then
		...
		if str:find("Named" .. name, nil, true) then name2 = "Named" .. name end

		for func_name in str:gmatch("<command name=\"(.-)\"/>") do
			local friendly = func_name:sub(3):gsub(name2 or name, "")

			if friendly ~= "Creates" then found[friendly] = functions[func_name] end
		end
...
for k, func_info in pairs(functions) do
	...
	for i, arg in ipairs(func_info.args) do
		...
		if i ~= #func_info.args then arg_line = arg_line .. ", " end
	end
	...
	if func_info.name:find("Gen%a-s$") then
		insert(
			"\t\t\tgl." .. nice:sub(0, -2) .. " = function() local id = ffi.new('GLint[1]') func(1, id) return id[0] end"
		)
	end
		...
		for friendly, info in pairs(object_functions) do
			...
			if object_info.name then
				...
				if not functions["gl" .. temp] then
					...
					if not functions["gl" .. temp] then temp = func_name end
				end
			...
			if functions["gl" .. func_name] then
				...
				if friendly == "Image2D" then print(func_name, arg_line2) end
				...
				if name == "Framebuffer" then
					if arg_line2:find("target") then
						...
						insert(
							"\t\t\t\tbind(self, \"GL_FRAMEBUFFER\") return gl." .. func_name .. "(" .. arg_line2 .. ")"
						)
					end
				else
