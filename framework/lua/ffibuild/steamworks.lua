if not vfs.IsFile("steam_api.json") then
	error(
		"please put steam_api.json in your data folder (" .. R("data/") .. "steam_api.json"
	)
end
...
do -- typedefs
	...
	for i, info in ipairs(json.typedefs) do
		if not blacklist[info.typedef] then
			...
			if info.type:find("(*)", nil, true) then
				local line = info.type:gsub("%(%*%)", function()
					return "(*" .. prepend .. info.typedef .. ")"
				end)
	...
	a(
		"typedef struct %s {} %s;",
		prepend .. "ISteamMatchmakingServerListResponse",
		prepend .. "ISteamMatchmakingServerListResponse"
	)
	a(
		"typedef struct %s {} %s;",
		prepend .. "ISteamMatchmakingPlayersResponse",
		prepend .. "ISteamMatchmakingPlayersResponse"
	)
	a(
		"typedef struct %s {} %s;",
		prepend .. "ISteamMatchmakingPingResponse",
		prepend .. "ISteamMatchmakingPingResponse"
	)
	a(
		"typedef struct %s {} %s;",
		prepend .. "ISteamMatchmakingRulesResponse",
		prepend .. "ISteamMatchmakingRulesResponse"
	)
end
	...
	local function add_fields(info, level)
		for _, field in pairs(info.fields) do
			...
			if type == "struct" or type == "union" then
				...
				if not pcall(ffi.typeof, field.fieldtype) then
					field.fieldtype = prepend .. field.fieldtype
				end
		...
		if not info.classname:find("::", nil, true) then
			...
			if info.params then
				...
				for i, arg in ipairs(info.params) do
					...
					if arg.paramtype ~= "const char *" then
						arg.paramtype = arg.paramtype:gsub("const ", "")
					end

					arg.paramtype = arg.paramtype:gsub("::", "_")

					if not pcall(ffi.typeof, arg.paramtype) then
						arg.paramtype = prepend .. arg.paramtype
					end

					args = args .. (
							"%s %s%s"
						):format(arg.paramtype, arg.paramname, i == #info.params and "" or ", ")
				end
			end
...
header = header:gsub("&", "*")
header = header:gsub(
	"struct " .. prepend .. "CSteamID %b{} " .. prepend .. "CSteamID",
	"uint64_t " .. prepend .. "CSteamID"
)
