...
function math2d.IsCoordinatesConvex(points)
	...
	while k + 1 < #polygons do
		...
		if (p.x * q.y - p.y * q.x) * winding < 0 then return false end
	end
...
function math2d.RoundedRectangleToCoordinates(x, y, w, h, radius_x, radius_y, resolution)
	...
	if w >= 0.02 then radius_x = math.min(radius_x, w / 2 - 0.01) end

	if h >= 0.02 then radius_y = math.min(radius_y, h / 2 - 0.01) end
...
do
	...
	function math2d.ArcToCoordinates(arc_mode, x, y, radius, angle1, angle2, points)
		...
		if angle < math.pi * 2 then points = points * angle / (2 * math.pi) end
		...
		if angle_shift == 0 then return end
...
do
	...
	local function line_(
		...
		if draw_overdraw then
			...
			if draw_mode == "triangle_strip" then extra_vertices = 2 end
		end
		...
		if draw_overdraw then
			...
			if mode == "none" then
				...
				if not is_looping then
					...
					spacer = (
							overdraw[overdraw_vertex_start + vertex_count - 1] - overdraw[overdraw_vertex_start + vertex_count - 3]
						)
					spacer:Normalize(pixel_size)
	...
	function math2d.CoordinatesToLines(coords, width, join, mode, pixel_size, draw_overdraw)
		...
		if mode == "miter" then
			return draw_mode,
			line_(mode, coords, count, count, width, pixel_size, draw_overdraw, draw_mode, join),
			nil
		elseif mode == "bevel" then
			return draw_mode,
			line_(
		...
		elseif mode == "none" then
			local vertices, overdraw_count = line_(
				mode,
				coords,
				count,
				2 * count - 4,
...
do
	...
	local function subdivide(polygons, k)
		if k <= 0 then return polygons end
