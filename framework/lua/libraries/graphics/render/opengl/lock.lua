...
local function range_overlaps(a, b)
	return a.start_offset < (
			b.start_offset + b.length
		)
		and
		b.start_offset < (
			a.start_offset + a.length
		)
end
...
local function wait(sync_obj)
	...
	while true do
		...
		if wait_ret == GL_WAIT_FAILED then error("uh oh") end
...
function render.WaitForLockedRange(lock_begin_bytes, lock_length)
	local test_range = {start_offset = lock_begin_bytes, length = lock_length}
	local swap_locks = {}
