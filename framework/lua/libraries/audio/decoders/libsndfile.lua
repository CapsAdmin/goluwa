...
if RELOAD then
	...
	local major_count = ffi.new("int[1]")soundfile.Command(nil, soundfile.clib.SFC_GET_FORMAT_MAJOR_COUNT, major_count, ffi.sizeof(major_count))major_count=major_count[0]
	local minor_count = ffi.new("int[1]")soundfile.Command(nil, soundfile.clib.SFC_GET_FORMAT_SUBTYPE_COUNT, minor_count, ffi.sizeof(minor_count))minor_count=minor_count[0]
