...
function structs.Register(META)
	...
	if ffi then
		...
		for prepend, number_type in pairs(number_types) do
			...
			for arg_i, arg in pairs(META.Args) do
				...
				for i, v in pairs(arg) do
					if i == 1 then
						if arg_i == 1 then arg_lines[i] = number_type .. " " end
		...
		for arg_i, arg in pairs(META.Args) do
			...
			if arg_i ~= #META.Args then arg_line = arg_line .. "," end
		end

		arg_line = arg_line .. ")"
		tbl_line = tbl_line .. "}"
		ctor = loadstring(
			"local META, setmetatable = ... return function" .. arg_line .. " return setmetatable(" .. tbl_line .. ", META) end"
		)(META, setmetatable)
	end

	if META.Constructor then
		structs[META.ClassName] = function()
			local self = ctor()
			self:Constructor()
			return self
		end
...
function structs.AddOperator(META, operator, ...)
	if operator == "tostring" then
		...
		for i in pairs(META.Args) do
			...
			if i ~= #META.Args then str = str .. ", " end
		end
...
function structs.Swizzle(META, arg_count, ctor)
	...
	for i = 1, arg_count do
		...
		if i ~= arg_count then index_args = index_args .. ".." end
	end
	...
	for i = 1, arg_count do
		...
		if i ~= arg_count then ctor_args = ctor_args .. "..', a.'.. " end
	end
