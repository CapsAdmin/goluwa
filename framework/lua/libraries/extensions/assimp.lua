...
local function parse_scene(scene, path, callback)
	if not scene then return nil, ffi.string(assimp.GetErrorString()) end
	...
	for i = 0, scene.mNumMeshes - 1 do
		...
		if mesh.mMaterialIndex > 0 then
			...
			for i = 0, mat.mNumProperties - 1 do
				...
				key = key:sub(2)
				val = val:sub(4)
				val = val:gsub("(.)", function(char)
					if char:byte() == 0 then return "" end
				end)

				if key == "mat.name" then sub_model.material.name = val end
