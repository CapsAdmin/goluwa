...
#define USE_OPENGL_330 1
#define mode_height_to_normal_330 1
...
vec4 ffilter(){
   ...
   vec4 textureColor = texture( layerA, uv.xy);
...
vec4 ffilter(){
    ...
    if(uv_scaling_mode == 1){
        ...
        if(corners_weights.x >= 0){
            texcoord.x = pow(uv.x,(corners_weights.x+1));
        }else{
            texcoord.x = 1+pow(1-uv.x,abs(corners_weights.x)+1);
            texcoord.x = 1 - texcoord.x;
        }
        if(corners_weights.y >= 0){
            texcoord.y = pow(uv.y,(corners_weights.y+1));
        }else{
            texcoord.y = 1+pow(1-uv.y,abs(corners_weights.y)+1);
...
vec4 ffilter(){
    ...
    vec4 color = texture( layerA, uv.xy);
    vec4 inversion;
    if(gui_image_type == 3){
...
vec4 ffilter(){
    ...
    vec4 colorA   = texture( layerA, uv.xy); // color
    vec4 colorB   = texture( layerB, uv.xy); // ao
    vec4 refColor = texture( layerC, uv.xy); // refcolor
...
vec4 ffilter(){
    ...
    vec4 colorA   = texture( layerA, uv.xy);
    vec4 blured   = texture( layerB, uv.xy);
...
vec4 ffilter(){
    ...
    vec4 colorA = texture( layerA, uv.xy);
    vec4 colorB = texture( layerB, uv.xy);
...
vec4 gauss_filter(sampler2D layer,float w, int radius, float depth){
    ...
    for(int i = -radius ; i <= radius ; i++){
            for(int j = -radius ; j <= radius ; j++){
                            ...
                            totalw += cw;
                            color   += texture(layer,uv.xy+depth*vec2(i,j)*dxy)*cw;
            }
    }
    return color/totalw;
}
...
vec4 ffilter(){
    ...
    if(gui_gauss_mask == 0){ // standard processing without masking
        ...
        // original color
        vec4  ocolor = texture(layerA,uv.xy);
        // mask texture
        float mask   = abs(float(gui_gauss_invert_mask) - clamp(texture(layerB,uv.xy).r,0,1));
        vec4 scolor  = texture(layerC,uv.xy); // blured image

        // draw mask on image
        if(gui_gauss_show_mask == true){
            vec2 grid = 2*abs(fract(uv.xy*10)-0.5);
...
float sobel_edge_detection(){
    ...
    #define UPDATE_SOBEL_VALUES(x, y, sobelMat) \
        (sobelMat)[0] += textureOffset(layerA,uv.st, ivec2( (x), (y) ) ) * sobel_kernel[ (x) + 1 ][ (y) + 1 ];   \
        (sobelMat)[1] += textureOffset(layerA,uv.st, ivec2( (x), (y) ) ) * sobel_kernel[ (y) + 1 ][ (x) + 1 ];
...
vec4 ffilter(){
    ...
    vec4 c1 =texture(layerA,uv.xy);
    vec4 c2 =texture(layerB,uv.xy);
    vec4 dc = c1-c2;
...
vec4 ffilter(){
    ...
    vec4 colorA      = texture( layerA, uv.xy); // oryginal texture (not blured)
    vec4 bluredBig   = texture( layerB, uv.xy);
...
vec4 ffilter(){
    ...
    if(gui_sharpen_blur > 0){
            vec4  color  = texture(layerA, uv.st);
...
vec4 ffilter(){
        ...
        vec4 color = 2*(texture( layerA, uv.xy) - 0.5);
...
vec4 ffilter(){
        ...
        vec2 newTexCoords = vec2(uv.xy-0.5) - vec2(gui_normal_mixer_pos_x,gui_normal_mixer_pos_y);
        // rotate
        ...
        vec3 normalA = 2*(texture( layerA, uv.xy).xyz - 0.5);
...
vec4 ffilter(){
    ...
    for(int i = 0 ; i < 3 ; i++){
        for(int j = 0 ; j < 3 ; j++){
            sobel_x   += texture(layerA,uv.xy+vec2(i-1,j-1)*dxy).r*sobel_kernel[i][j];
            sobel_y   -= texture(layerA,uv.xy+vec2(i-1,j-1)*dxy).r*sobel_kernel[j][i];
        }
    }
...
vec4 ffilter(){
#endif
    vec3 normal   = normalize(texture(layerA,uv.xy).rgb-0.5);
...
vec4 ffilter(){
    ...
    for(int i = -radius ; i <= radius ; i++){
    for(int j = -radius ; j <= radius ; j++){
        vec2 coords = vec2(uv.xy+vec2(i,j)*dxy);
    ...
    vec3 n      = normalize(texture(layerA,uv.xy).xyz*2-1);//get normal
    float slope = (1.0/(exp(+10*gui_mix_normals* length(n.xy))+1)); // slope based blending param

    vec4 a = texture(layerA,uv.xy);//processed normal image
    vec4 b = texture(layerB,uv.xy);//normal image obtained from sobel filter
...
vec4 ffilter(){
    ...
    vec3 normalA     = normalize(texture(layerA,uv.xy).rgb - 0.5);
    vec3 normalB     = normalize(texture(layerB,uv.xy).rgb - 0.5);
    vec3 normalC     = normalize(texture(layerC,uv.xy).rgb - 0.5);
    vec3 normalD     = normalize(texture(layerD,uv.xy).rgb - 0.5);
...
vec4 ffilter(){
#endif
	vec3 p 		= getPosition(uv.xy); // pozycja w ukladzie oka
	vec3 n 		= getNormal(uv.xy); // normalna
	float ao      = 0.0f;
	for(int i = -gui_ssao_no_iters ; i <=  gui_ssao_no_iters ; i++){
		for(int j = -gui_ssao_no_iters ; j <=  gui_ssao_no_iters ; j++){
				ao += doAmbientOcclusion(uv,vec2(i,j)*dxy*g_scale, p, n);
	}}
...
vec4 ffilter(){
    ...
    vec4 height     = texture( layerA, uv.xy);
    ...
    for(int x  = -(radius-1) ; x <= radius-1 ; x++){
        for(int y  = -(radius-1) ; y <= radius-1 ; y++){
            ave_color += texture( layerA, uv.xy + vec2(x,y)*dxy);
            no_samples++;
    }}
...
vec4 ffilter(){
    ...
    float ave       = texture( layerB, uv.xy ).r;


    float curvature = 0;
    for(int x  = -10 ; x <= 10 ; x++){
        for(int y  = -10 ; y <= 10 ; y++){
          float val = texture( layerA, uv.xy + depth * vec2(x,y) * dxy ).r;
...
vec4 ffilter(){
    ...
    if(gui_roughness_color_method == 0){ // angle based A
        ...
        vec3 colorA     = normalize(texture( layerA, uv.xy).rgb-0.5);
        ...
        vec3 colorA     = normalize(texture( layerA, uv.xy).rgb);
        ...
        vec3 colorA     = (texture( layerA, uv.xy).rgb);
...
vec4 ffilter(){
    ...
    vec4 I  = texture( layerA, uv.xy);
    vec4 M  = texture( layerB, uv.xy);
...
vec4 ffilter(){
    ...
    if(!gui_grunge_brandomize){
           return texture( layerA, uv.xy*rand_scale);
    }
...
vec4 ffilter(){
#endif
    vec2 normal = texture( layerB, uv.xy).st - 0.5;
    vec4 color  = texture( layerA, uv.xy + normal*grunge_normal_warp*0.05);
...
void main() {   
    ...
    vec3 materialColor = texture( materialTexture, uv.xy).rgb;
    ...
    if(material_id >= 0){// if current ID is different than -1
        ...
        FragColor = texture( layerA, uv.xy);
...
FRAGMENT = FRAGMENT:gsub("(uniform.-;)", function(def) 
	def = def:gsub("%s+", " ")
	local _, type, name = unpack(def:explode(" "))
	name = name:sub(0, -2)
	
	if type == "sampler2D" then
		--type = render.GetWhiteTexture()
	end
...
local tex = Texture("brickwall053d.png")

shader.quad_draw_mode = 1

event.AddListener("PostDrawMenu", "test", function()
	for k,v in pairs(variables) do
		if v == "float" then
			shader[k] = math.randomf(-1,1)*100
		elseif v == "int" then
			shader[k] = 0
		elseif v == "vec3" then
			--shader[k] = Vec3():GetRandom()
		end
	end
	render.SetShaderOverride(shader)
	shader.materialTexture = tex
