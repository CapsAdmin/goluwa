...
local map_datar = { -- Data processing, register.
	...
	[0] = {
		...
		mask = 1,
		[0] = map_logsr,
		{shift = 21, mask = 1, [0] = map_addsubsh, map_addsubex},
	},
	{
		shift = 21,
		mask = 15,
		...
		map_csel,
		false,
		{shift = 30, mask = 1, [0] = map_data2s, map_data1s},
		false,
		map_data3s,
		map_data3s,
		map_data3s,
...
local map_lrl = { -- Load register, literal.
	shift = 26,
	mask = 1,
	[0] = {shift = 30, mask = 3, [0] = "ldrDwB", "ldrDxB", "ldrswDxB"},
	{shift = 30, mask = 3, [0] = "ldrDsB", "ldrDdB"},
}
local map_lsriind = { -- Load/store register, immediate pre/post-indexed.
	...
	},
	{
		shift = 26,
		mask = 1,
		[0] = {shift = 22, mask = 3, [0] = "strDwzL", "ldrDwzL", "ldrswDxzL"},
		{shift = 22, mask = 3, [0] = "strDszL", "ldrDszL"},
	},
	{
		shift = 26,
		mask = 1,
		[0] = {shift = 22, mask = 3, [0] = "strDxzL", "ldrDxzL"},
		{shift = 22, mask = 3, [0] = "strDdzL", "ldrDdzL"},
	},
}
local map_lsriro = {
	shift = 21,
	...
	[0] = { -- Load/store register immediate.
		...
		[0] = { -- Unscaled immediate.
			...
			[0] = {
				shift = 30,
				mask = 3,
				[0] = {shift = 22, mask = 3, [0] = "sturbDwK", "ldurbDwK"},
				{shift = 22, mask = 3, [0] = "sturhDwK", "ldurhDwK"},
				{shift = 22, mask = 3, [0] = "sturDwK", "ldurDwK"},
				{shift = 22, mask = 3, [0] = "sturDxK", "ldurDxK"},
			},
		},
		map_lsriind,
		false,
	...
	{ -- Load/store register, register offset.
		...
		[2] = {
			...
			[0] = {
				...
				{shift = 22, mask = 3, [0] = "strDwO", "ldrDwO", "ldrswDxO"},
				{shift = 22, mask = 3, [0] = "strDxO", "ldrDxO"},
			},
			{
				shift = 30,
				mask = 3,
				[2] = {shift = 22, mask = 3, [0] = "strDsO", "ldrDsO"},
				[3] = {shift = 22, mask = 3, [0] = "strDdO", "ldrDdO"},
			},
		},
	},
}
local map_lsp = { -- Load/store register pair, offset.
	...
	[0] = {
		...
		{shift = 26, mask = 1, "stpDzAzdP"},
		{shift = 26, mask = 1, [0] = "stpDzAzxP"},
	},
	{
		shift = 30,
		mask = 3,
		...
		{shift = 26, mask = 1, [0] = "ldpswDAxP", "ldpDzAzdP"},
		{shift = 26, mask = 1, [0] = "ldpDzAzxP"},
	},
}
local map_ls = { -- Loads and stores.
	...
	[0x10] = map_lrl,
	[0x30] = map_lsriro,
	[0x20] = {shift = 23, mask = 3, map_lsp, map_lsp, map_lsp},
	[0x21] = {shift = 23, mask = 3, map_lsp, map_lsp, map_lsp},
	[0x31] = {
		shift = 26,
		mask = 1,
		[0] = {
			shift = 30,
			mask = 3,
			[0] = {shift = 22, mask = 3, [0] = "strbDwzU", "ldrbDwzU"},
			{shift = 22, mask = 3, [0] = "strhDwzU", "ldrhDwzU"},
			{shift = 22, mask = 3, [0] = "strDwzU", "ldrDwzU"},
			{shift = 22, mask = 3, [0] = "strDxzU", "ldrDxzU"},
		},
		{
			shift = 30,
			mask = 3,
			[2] = {shift = 22, mask = 3, [0] = "strDszU", "ldrDszU"},
			[3] = {shift = 22, mask = 3, [0] = "strDdzU", "ldrDdzU"},
		},
	},
}
	...
	{ -- 001
			...
			mask = 1,
			{
				...
				{ -- FP conditional compare.
					...
					[0] = {
						shift = 23,
						mask = 1,
						[0] = {shift = 4, mask = 1, [0] = "fccmpNMVCf", "fccmpeNMVCf"},
					},
				},
				...
				{ -- FP conditional select.
					shift = 31,
					mask = 1,
					[0] = {shift = 23, mask = 1, [0] = "fcselDNMCf"},
				},
			},
		},
		{ -- FP data-processing, 3 sources.
			...
			[0] = {
				shift = 15,
				mask = 1,
				[0] = {shift = 21, mask = 5, [0] = "fmaddDNMAf", "fnmaddDNMAf"},
				{shift = 21, mask = 5, [0] = "fmsubDNMAf", "fnmsubDNMAf"},
			},
		},
	},
}
...
local function fmt_hex32(x)
	if x < 0 then return tohex(x) else return format("%x", x) end
end
...
local function prefer_bfx(sf, uns, imms, immr)
	if imms < immr or imms == 31 or imms == 63 then return false end

	if immr == 0 then
		if sf == 0 and (imms == 7 or imms == 15) then return false end
...
local function disass_ins(ctx)
	...
	for p in gmatch(pat, ".") do
		...
		elseif p == "Y" then
			...
			if altname and (hw == 0 or x ~= 0) then name = altname end
		elseif p == "L" then
		...
		elseif p == "O" then
			...
			if opt == 3 then
				if s == 0 then x = x .. "]" else x = x .. ", lsl #" .. sz .. "]" end
		...
		elseif p == "2" then
			...
			if altname then
				...
				elseif immr == 0 and x == 7 then
					...
					if sf ~= 0 then operands[n - 1] = gsub(operands[n - 1], "x", "w") end
		...
		elseif p == "X" then
			...
			if
				...
				and
				band(rshift(op, second0 and 5 or 0), 31) == 31
			then
		...
		elseif p == "R" then
			...
			if x == 0 then x = nil else x = "lsl #" .. x * 16 end
		elseif p == "z" then
			local n = #operands
...
return {create = create, disass = disass, regname = regname}