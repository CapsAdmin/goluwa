...
test(fs.set_current_directory, "TEST/TEST").expect(true)
    ...
    test(function() return f:read("*all") end).expect("hello world")
    f:close()
...
test(fs.copy, "golwua.cmd", "goluwa2.cmd").expect_compare(function() return fs.Read("goluwa.cmd") == fs.Read("goluwa2.cmd") end)