...
function META:IsFunctionStatement()
	return self:IsValue("function") or
		(
			self:IsValue("local") and
			self:GetTokenOffset(1).value == "function"
		)
		or
		(
			self:IsValue("async") and
			self:GetTokenOffset(1).value == "function"
		)
		or
		(
			self:IsValue("local") and
			self:GetTokenOffset(1).value == "async" and
			self:GetTokenOffset(2).value == "function"
		)
end
...
function META:IsAnonymousFunction()
	return self:IsValue("function") or self:IsValue("do") or self:IsValue("async")
end
