	...
	do -- grammar rules
		if syntax.UTF8 then
			function syntax.GetCharacterType(char)
				return syntax.CharacterMap[char] or
					(
						syntax.TokenizerSetup.FallbackCharacterType and
						char:byte() > 128 and
						syntax.TokenizerSetup.FallbackCharacterType
					)
			end
		else
			function syntax.GetCharacterType(char)
		...
		function syntax.IsValue(token)
			return token.type == "number" or
				token.type == "string" or
				syntax.KeywordValues[token.value]
		end
