...
[==[
	...
	stdenv.mkDerivation {
		...
		src = ./.;
		buildInputs = [ gcc ]==] .. (data.build_inputs and (" " .. data.build_inputs .. " ") or "") .. "(" .. data.package_name .. ")" .. [==[ ];
		]==] .. build_phase .. [==[
		installPhase = ''
			mkdir $out;
			mkdir $out/include;
			...
			if not num then
				...
				do
					...
					if found then
						...
						if type(val) == "string" and (val:sub(#val, #val):lower() == "u" or val:sub(#val, #val):lower() == "l") then
							val = val:sub(0, -2)
						end
