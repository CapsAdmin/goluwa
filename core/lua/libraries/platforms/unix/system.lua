...
do
	...
	function system.SetConsoleTitleRaw(str)
		if repl and repl.SetConsoleTitle then return repl.SetConsoleTitle(str) end
	...
	function system.FindFirstTextEditor(os_execute, with_args)
		for _, v in pairs(text_editors) do
			if io.popen("command -v " .. v.name):read() then
				...
				if v.terminal then cmd = "x-terminal-emulator -e " .. cmd end

				if with_args then cmd = cmd .. " " .. v.args end

				if os_execute then cmd = cmd .. " &" end
...
function system._OSCommandExists(cmd)
	if io.popen("command -v " .. cmd):read("*all") ~= "" then return true end
end