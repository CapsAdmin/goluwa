...
local function generic_check(ret)
    if ret == 0 then
        return true
    end
...
local function generic_function(lib_name, cdef, alias, skip_error_handling)
    ffi.cdef(cdef)
    ...
    if skip_error_handling then
        module[func_name] = function(...)
            return func(...)
        end
    else
        module[func_name] = function(...)
            return generic_check(func(...))
        end
    end
end
...
if jit.os == "Windows" then
    ...
    do
        ...
        function module.socket_blocking(fd, b)
            return generic_check(lib.ioctlsocket(fd, FIONBIO, ffi.new("int[1]", b and 0 or 1)))
        end
    end
else
    ffi.cdef([[
...
generic_function("accept", "SOCKET accept(SOCKET s, struct sockaddr *, unsigned int *);", nil, true)
...
generic_function("send", "int send(SOCKET s, const char* buf, int len, int flags);")
generic_function("sendto", "int sendto(SOCKET s, const char* buf, int len, int flags, const struct sockaddr* to, int tolen);")
