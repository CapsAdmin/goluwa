...
local http_headers = function(request)
	...
	if not request:match(".*HTTP/1%.1") then return headers end
	...
	for line in request:gmatch("[^\r\n]*\r\n") do
		...
		if name and val then
			...
			if not name:match("sec%-websocket") then val = val:lower() end
...
local accept_upgrade = function(request, protocols)
	...
	if
		headers["upgrade"] ~= "websocket" or
		not headers["connection"] or
		not headers["connection"]:match("upgrade")
		or
